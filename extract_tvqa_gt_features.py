...
def get_tvqa_plus_gt_subtitle(json_path,vid_name,ts):
    subtitle_file = read_jsonfile(json_path)
    if vid_name in subtitle_file:
        sub_text = subtitle_file[vid_name]["sub_text"]
        sub_time = subtitle_file[vid_name]["sub_time"]
        sentences = sub_text.split("<eos>")
        start = -1
        end = -1
        for i, data in enumerate(sub_time):
            if start<0 and data>ts[0]:
                start = i
            if end<0 and data>ts[1]:
                end = i
        if start > 0:
            start = start - 1
        else:
        ...
        res = " ".join(sentences[start:end+1])
        print(len(sentences), len(sub_time))
        print(sentences)
        print("res subtitle: "+res)
        print(ts)
        print(sub_time)
        print(start, end)
...
def get_tvqa(dataset_path,start,end):
    ...
    for i, data in enumerate(tvqa_plus_annotations[start:end]):
        ...
        answer_index = int(data["answer_idx"])
        ts = data["ts"]
        subtitle = get_tvqa_plus_gt_subtitle("TVQA/tvqa_plus_subtitles/tvqa_plus_subtitles.json",video,ts)
...
def extract_tvqa_features():
    ...
    for i, data in enumerate(tvqa_dataset):
        ...
        path = "TVQA/" + "val_features_soft_gt/" + str(start+i) + ".pth"
