...
class DataNode():
    def __init__(self,video="",question="",candidate_answers=[],answer_index=0,subtitle="",visual_text="",keywords="",question_type=0):
        self.video = video
...
def get_features(data):
    ...
    question_type = torch.Tensor([data.question_type])

    features = FeatureNode(allqa_features,text_features,visual_features,keywords_features, answer_index, question_type)
    return features
...
def get_lqa(dataset_path):
    ...
    for key in lqa_json:
        ...
        for q in data["questions"]:
            ...
            if question_type == "what" or question_type == "whaat" or question_type == "whats" or question_type == "what's" or question_type == "whare" or question_type == "waht" or question_type == "waht's":
                question_type = 0
            ...
            datanode = DataNode(video,question,candidate_answers,answer_index,subtitle,visual_text,keywords,question_type)
            # print(video,question,candidate_answers,answer_index,subtitle,visual_text,keywords)
    ...
    with open(dataset_path, "r", encoding="utf-8") as tvqa_qa_file:
        ...
        for data in jsonlines.Reader(tvqa_qa_file):
            if i>=start and i<end:
                ...
                # gt_subtitle = get_old_tvqa_gt_subtitle("TVQA-/tvqa_subtitles/", video, ts)
...
def extract_lqa_features():
    ...
    lqa_dataset = get_lqa("LifeQA/lqa_dev.json")
    print("Succeed in loading dataset!!!!!")
    ...
    for i, data in enumerate(lqa_dataset):
        ...
        path = "LifeQA/" + "dev_features/" + str(start+i) + ".pth"
...
if __name__ == '__main__':
    ...
    # extract_old_tvqa_features()
    ###add keyword extraction
    ###chage qa to [cls] q [sep] a [sep]
    #### add lqa  answer index  question
